...
library(here)
library(shiny)
shiny_url <- "https://oahbxj-inge0r-holtman.shinyapps.io/"
...
question <- function (
  ...
  # browser()
  correct <- glue("{random_praise()}\n{correct}") # if_else(correct != "Correct!", correct, random_praise())
  incorrect <- glue("{random_encouragement()}\n{incorrect}") # incorrect <- if_else(incorrect != "Incorrect", incorrect, random_encouragement())
...
here <- function(...) {
  v_args <- list(...)
  v_args_ext <- c(list("tutorials", "03-models"), v_args)
  case_when(
