...
# https://oferengel-posit.shinyapps.io/03-08-lesson/
...
1. For our explication and demonstration, we will use `babies`, a data set originated from the Child Health and Development Studies. It is a data frame with 1236 observations and eight variables. For details see [package documentation](http://openintrostat.github.io/openintro/reference/babies.html).
...
Now you will fit a multiple linear regression model (MLR model) yourself. The dataset `mariokart` you are going to use is already loaded in your workspace. Have a first look with `glimpse()` at the data:
...
```{r ex2-mariokart-hint-1}
# add predictions to grid
price_hats <- augment(mod, newdata = grid)
```

```{r ex2-mariokart-hint-2}
# add predictions to grid
price_hats <- augment(mod, newdata = grid)
...
```{r ex3-mariokart-hint-1}
# load plotly package
library(plotly) 

```

```{r ex3-mariokart-hint-2}
# load plotly package
library(plotly) 
...
You fit a multiple regression model for the total auction price of an item in the `mariokart` data set as a function of the starting price and the duration of the auction. You computed the coefficients. Now choose the correct interpretation of the `duration` variable.


```{r mc2}
question("This interpretation is correct …",
  ...
  answer("For each additional dollar of starting price, the expected final price increases by $0.23, after controlling for the duration of the auction.", correct = TRUE, message="Remember, the inclusion of another variable in your model is sometimes referred to as 'controlling for' that variable."),
  answer("The duration of the auction is a more important determinant of final price than starting price, because the coefficient is larger."),
  answer("The average auction lasts 51 days."),
  allow_retry=TRUE
)
```
...
```{r ex5-mariokart-hint-1}
# load plotly
library(plotly)
```


```{r ex5-mariokart-hint-2}
# load plotly
library(plotly)
...
```{r ex6-mariokart, exercise=TRUE}
lm(formula = ___ ~ ___ + ___ + ___, data = ___)
```

```{r ex6-mariokart-solution}
...
This time you will throw even more variables into your `mariokart` model, including the number of bids in each auction (`n_bids`) and the number of `wheels`. 

```{r ex7-mariokart, exercise=TRUE}
lm(formula = ___ ~ ___ + ___ + ___ + ___ + ___, data = ___)
```


```{r ex7-mariokart-solution}
...
question("Choose the correct interpretation of the coefficient on the number of wheels",
  answer("The average number of wheels is 6.72.", message="We can't infer that from our data."),
  answer("Each additional wheel costs exactly $6.72.", message="Exactly' is rarely used in statistics!"),
  answer("Each additional wheel is associated with an increase in the expected auction price of $6.72.", message="Right idea, but...The coefficient on `wheels` represents how the number of wheel affects the price when you take into account all the other variables in the model."),
  answer("Each additional wheel is associated with an increase in the expected auction price of $6.72, after controlling for auction duration, starting price, number of bids, and the condition of the item.", correct = TRUE), 
  allow_retry=TRUE
)
```
