...
quiz(
  caption = "", 
  question("The `mpg` dataset consists of several columns of data. What is true about those columns (choose more than one)?",
    allow_retry = TRUE, random_answer_order = TRUE,
    answer("`drv` and `class` are continuous variables.", message = "Nope, try again!"),
    answer("`displ` measures engine's volume (in litres)", correct = TRUE),
    answer("`cty` and `hwy` measure fuel efficiency", correct = TRUE),
    answer("`manufacturer` is a categorical variable", correct = TRUE)
  )
)
...
question("What geom would you use to draw a line chart? A boxplot? A histogram? An area chart?",
