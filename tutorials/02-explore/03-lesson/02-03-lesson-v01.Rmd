...
question("What happens when you remove the `group_by` clause from the code above?", 
         ...
         answer("we get the mean life expectency across all residents in the USA"), 
         answer("we get the mean of the life expectencies across all counties in the USA (the mean of county means)", correct=TRUE, message="That's correct. Each row represents one county in the USA. If we take the mean of the rows, we are effectively averaging across counties and not across states."), 
...
The simplest measure is  the **range** of the data: the distance between the maximum and the minimum.
...
Consider the following data-points: $X=\{10, 10, 15, 18, 22\}$. Check that the sample mean of this set is $\bar{X}=15$.
...
$$
\text{distance}_i=X_i - \bar{X}=\{10 - 15, 10- 15, 15 - 15, 18 - 15\cdots \}=\{-5, -15, 0, 3, \cdots \}
$$
...
Though simple to use and intuitive as a measure of spread, the two MADs have some unwanted properties. First, the absolute value of a number has no derivative at zero, which makes it mathematically inconvenient to use. In addition, the mean absolute deviation from the **mean** can be the same as the mean absolute deviation from other points along the continuum, so as a measure of spread, MAD is not uniquely associated with the mean. This becomes a problem, when we would like our measure of dispersion to be a mathematical optimum of some objective function.
...
quiz(caption = "", 
     question_checkbox("In a previous question, we explored a set consisting of ninety-nine times the value zero, and one time the value 100 $X=\\{0,0,0,0,\\ldots,100 \\}$. We found that the median of this set is zero while its mean is 1.<br/><br/> Imagine that a number is chosen at random from this set. Your job is to guess this number. Making a mistake incurs a cost, equal to the difference between your guess and the real number. <br/><br/>For example, if you guess 50 and the real number is 100, the cost is the difference between 100 and 50, which is 50. You want to minimize your (average) cost. <br/><br/>Select the correct answer:", 
...
### Variation and standard deviation in R

To calculate measures of spread in R, we'll first define the center as the mean and quantify the distance from the mean by taking the difference between each observation and that mean. 
...
We'd like to reduce all of these differences to a single measure of variability, and one option is to add them up. But as we saw before, if we do that the positives and negatives will cancel each other. To avoid that (and also to give higher weight to deviations from the mean that are larger), we square each difference.
...
This new measure has an undesirable property: it will just keep getting bigger the more data that you add. You can fix this by looking at the average square deviation, that is, dividing this measure by the number of observations, `r nrow(life_ma)`. Now, the quantity is a useful measure found by the following steps: 
...
To calculate it in R, we simply use the function `sd()`. The sample standard deviation is more convenient than the variance because it is in the same units as the original data. In this case we can say that the standard deviation of the `r nrow(life_ma)` counties' life expectancies is `r life_ma_summary$sd` years. 
...
One last measure of spread that is good to know  is the **interquartile range**, or **IQR**. This is the distance between the two numbers that cut-off the middle 50% of your data. This should sound familiar from the discussion of boxplots: the height of the box is exactly the IQR. We can either calculate the first and third quartiles using the `quantile()` function and take their difference...
...
question("Which set of measures of spread would be *worst* for describing the two distributions shown here?",
  ...
  correct = "The variance and the range are measures that are the most sensitive to extreme values, therefore we would probably avoid them in this case.",
  allow_retry = TRUE
)
```
...
The `gap2007` dataset is an excerpt of the [Gapminder](https://en.wikipedia.org/wiki/Gapminder_Foundation) data on life expectancy, GDP per capita, and population by country.
