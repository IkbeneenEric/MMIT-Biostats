---
title: "2.2 Exploring Numerical Data"
output: 
  learnr::tutorial:
    progressive: true
    allow_skip: true
...
```{r wbw-15, exercise = TRUE}
# Set the binwidth to 15
ggplot(data = cars, aes(x = hwy_mpg)) +
  geom_histogram(binwidth = ___)
```
...
```{r ex6-hint}
Does a distribution appear to have outliers? If so, a boxplot is more appropriate. Does a distribution have multiple modes? If so, a density plot is more appropriate.  
```

```{r ex6-solution-dont-use, echo=FALSE, eval=FALSE}
# Create plot of city_mpg
...
question("Boxplot or density plot?",
  answer("For the `city_mpg` variable,  a boxplot is most suitable", message = "That's right!", correct = TRUE),
  answer("For the `city_mpg` variable,  a density plot is most suitable", message = "Notice this variable has many outliers"),
  answer("For the `width` variable,  a boxplot is most suitable", message="notice that this variable has many small modes (peaks and troughs)"),
  answer("For the `width` variable,  a density plot is most suitable", message = "Very good :-)", correct = TRUE),
  allow_retry = TRUE
)
```
...
We can use the `count()` function to find the unique values for the variable `ncyl` and also see how many observations there are for each value. 

```{r ex-cars, exercise = TRUE}
cars |>
  count(___)
```

That's probably more possible levels of `ncyl` than you might think! Particularly, the `ncly` of -1 should catch your eye! (Perhaps this is an error in data entry?)

Here, we're going to make a new dataset that only contains the most common values of `ncyl`.
...
```{r ex2-setup, echo=FALSE}
common_cyl <- cars |>
  filter(ncyl %in% c(4, 6, 8))
```

```{r, ref.label="ex2-setup", echo=TRUE}
...
```{r mc1}
question("Which of the following interpretations of the plot is valid (choose more than one)?",
  answer("The highest mileage cars have 4 cylinders.", 
         correct = TRUE),
  answer("The typical 4 cylinder car gets better mileage than the typical 6 cylinder car, which gets better mileage than the typical 8 cylinder car.", 
         correct = TRUE),
  answer("Most of the 4 cylinder cars get better mileage than even the most efficient 8 cylinder cars.", correct = TRUE),
  answer("The variability in mileage of 8 cylinder cars is similar to the variability in mileage of 4 cylinder cars.", message = "Since its distribution is much narrower, the variability in mileage of 8 cylinder cars seem much smaller than that of 4 cylinder cars."),
  allow_retry = TRUE
)
```
