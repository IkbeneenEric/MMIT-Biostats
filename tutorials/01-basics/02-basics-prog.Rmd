...
Good job! The function `rnorm` has three arguments: 
- `n` is the first argument, and it specifies the number of random normal variables to generate.
- `mean` and `sd` are the second and third arguments, describing the mean and the standard deviation of the distribution to generate the random values with. Notice that these argument have default values. This means that if we do not provide the value of one of those arguments, they will be assigned those default values. 
...
question(
  ...
  allow_retry = TRUE, 
  random_answer_order = TRUE, 
  correct = "When a sequence of characters is enclosed within \"quotes\", it is of the character data type"
  )
```
...
Although we would expect that the answer would be exactly zero, the calculation is returning a vanishingly tiny number. 
For example, if the calculation resulted in the number `4.440892e-16`, that is equivalent to `0.000000000000000440892`, where the digital point moves sixteen digits to the left.
...
question(
  'How many types of data can you put into a single vector?',
         correct = "All the items in a vector must be of the same data type. Different types cannot be combined within the same vector.",
...
```{r lists4-question, echo = FALSE}
question(
  "Choose the correct statements regarding data frames:",
  type = "checkbox",
  answer("All items in a column belong to one vector", correct = TRUE),
  answer("All items in a row belong to one vector", correct = FALSE),
  answer("All items in a column are of the same data type", correct = TRUE),
  answer("All items in a row are of the same data type", correct = FALSE),
  correct = "Columns (variables) are a vector, and so all their values have the same data type. They are usually either numerical or categorical.",
  allow_retry = TRUE,
  random_answer_order = TRUE
)
```
