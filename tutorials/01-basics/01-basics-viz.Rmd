...
hdr_path <- case_when(
    ...
    file.exists(here::here("tutorials", "01-basics", "R", "header-include.R")) ~ here::here("tutorials", "01-basics", "R", "header-include.R")
  )
...
```{r quiz-rows-columns}
  question(
    "how many observations and how many variables are in the `hsb2` dataset? ",
    type = "checkbox",
    answer("234 variables", message = "Nope, try again!"),
    answer("234 observations", correct = TRUE),
    answer("11 variables", correct = TRUE),
    answer("11 observations", message = "Nope, try again!"), 
    correct = "Well done! The ROWS in a dataset are known as observations and the COLUMNS are the variables."
    )
...
```{r q-mpg-help}
question(
    "The `mpg` dataset consists of several columns of data. Which statement is UNTRUE about those columns?",
    answer("`drv` and `class` are numeric/continuous variables.", correct = TRUE),
    answer("`displ` measures engine's volume (in litres)"),
    answer("`cty` and `hwy` measure fuel efficiency"),
    answer("`manufacturer` is a categorical variable"), 
    correct = "These two variables are decidedly categrorical and not numeric/continuous."
  )
...
The `mpg` dataset is a subset of the fuel economy data [provided by the EPA](https://fueleconomy.gov/). Among the variables in `mpg` are:
...
Make a scatterplot showing how the variable `cty` (efficiency indicator in the city, map to the x-axis) determines the  `hwy` (efficiency on the highway, map to the y-axis).
...
```{r q-mpg-datatypes}
question(
    "The efficiency in the city `cty` is associated with that on the highway `hwy`. Which statement is TRUE (choose more than one)?",
    type = "checkbox", 
    answer("20 miles/gallon in the city is associated with about 28 miles/gallon on the highway", correct = TRUE),
    answer("miles/gallon in the city is generally higher than on the highway"),
    answer("miles/gallon in the city is generally lower than on the highway", correct = TRUE), 
    correct = "Cars are less efficient in the city than on the highway, which is why `cty` is generally lower than `hwy`."
  )
```
...
```{r aes-shapes, echo = FALSE, asp = 1/4, out.width = "100%"}
ggplot() +
...
**Hint:** Be sure to remove all of the underlines and hashtags from the code. Replace the underline with the following expression `color = class`.
</div>
...
# Map cty to shape: this will not work!
# ggplot(data = mpg) + 
...
Very nice! ggplot treats continuous and categorical variables differently. Noteably, ggplot supplies a blue gradient when you map a continuous variable to color, and ggplot will not map continuous variables to shape.
...
ggplot2 provides over 30 geom functions that you can use to make plots, and extension packages provide [even more](https://exts.ggplot2.tidyverse.org/gallery/). You'll learn how to use these geoms to explore data in the [Visualize Data]() primer. 
...
```{r q-predict01, echo=FALSE}

question(
  "What geom would you use to draw a line chart? A boxplot? A histogram? An area chart?",
  answer("geom_linechart(), boxplot(), geom_histogram(), geom_area()"),
  answer("geom_line(), geom_boxplot(), geom_histogram(), geom_areachart()"),
  answer("geom_line(), geom_boxplot(), geom_histogram(), geom_area()", correct = TRUE)
)
...
question(
  ...
  answer("chooses a method for calculating the smooth line", 
         message = "That is what the `method` argument of `geom_smooth()` does."),
...
There is much more to ggplot2 and Data Visualization than we have covered here. We will cover them in future tutorials!
...
if (file.exists(here("01-basics", "images", "Submit-hash.jpg")))
  knitr::include_graphics(here("01-basics", "images", "Submit-hash.jpg"))
```
