...
```{r hsb2-mutate-num-char-cat-hint}
"Remember, to filter out observations, the not equal to (`!=`) comparison comes in handy!"
```
...
```{r mccp5-hint}
 "Take a look at the table that's conditional on gender!"
```
...
2. use the `mutate()` function to add the changed `align` variable
