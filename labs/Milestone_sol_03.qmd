...
knitr::include_graphics(here("images/lab3-penguins.png"))

# Bill length and bill depth
knitr::include_graphics(here("images/lab3-culmen_depth.png"))

# Map of the three palmer archipelego
knitr::include_graphics(here("images/lab3-islands.png"))
...
# SOLUTION REMOVE
label(penguins$body_mass_g) <- "Body mass"
units(penguins$body_mass_g) <- "gram"

label(penguins$sex)         <- "Sex"

label(penguins$flipper_length_mm) <- "Flipper length"
units(penguins$flipper_length_mm) <- "mm"
...
table1(~ sex + body_mass_g + flipper_length_mm + bill_length_mm + bill_depth_mm + island | species, 
       data=penguins, 
...
#| echo: false
knitr::include_graphics(here("images/lab3-mass-depth.png"))
...
# SOLUTION REMOVE
M1 <- lm(body_mass_g ~ bill_depth_mm, data = penguins)
...
#| echo: false
knitr::include_graphics(here("images/lab3-lm-summary.jpg"))

```

Now fill in the blanks in this formal description of your results.

> A linear regression model was used to test the association between ______ (gram) and ______ (mm) using data from n = ______ Penguins. ______% of the variation in ______ was explained by ______ ($R^2$= ______). There was a significant ______ (positive/negative) association between ______ and ______ (B = ______; 95% CI = ______, ______; p < .001). On average, for every 1-______ difference in ______, penguins differ in mean ______ by ______ ______.
...
# SOLUTION REMOVE
M2 <- lm(body_mass_g ~ bill_depth_mm + species, data = penguins)
...
> A linear regression model was used to test the association between ______ (gram) and ______ (mm), while conditioning on the penguin species and using data from n = ______ Penguins. ______% of the variation in ______ was explained by ______ ($R^2$= ______). There was a significant ______ (positive/negative) association between ______ and ______ (B = ______; 95% CI = ______, ______; p \< .001). On average, for every 1-______ difference in ______, penguins differ in mean ______ by ______ ______.
...
knitr::include_graphics(here("images/lab3-parallel-slope.png"))
...
augment(M2, interval = "confidence") |> 
  ...
  geom_line(aes(bill_depth_mm, .fitted, color = species)) + 
  geom_ribbon(aes(x = bill_depth_mm, ymin = .lower, ymax = .upper, fill = species), alpha = .5) + 
  labs(
    title = "Modelling body mass: parallel slopes model", 
    x = "Bill depth (mm)", 
    y = "body mass (g)") + 
...
# ggsave(here("images/lab3-parallel-slope.png"))
...
knitr::include_graphics(here("images/lab3-no-parallel-slope.png"))
...
# SOLUTION REMOVE
M3 <- lm(body_mass_g ~ bill_depth_mm + species + bill_depth_mm * species, data = penguins)

augment(M3, interval = "confidence") |> 
  ggplot() + 
  geom_point(aes(bill_depth_mm, body_mass_g, color = species)) + 
  geom_line(aes(bill_depth_mm, .fitted, color = species)) + 
  geom_ribbon(aes(x = bill_depth_mm, ymin = .lower, ymax = .upper, fill = species), alpha = .5) + 
  labs(
    title = "Modelling body mass: interaction model", 
    subtitle = "Interaction = no-parallel slopes",
    x = "Bill depth (mm)", 
    y = "body mass (g)") + 
  scale_x_continuous(breaks = 13:21) + 
  theme_classic() + 
...
# ggsave(here("images/lab3-no-parallel-slope.png"))
...
-   Model 1 vs Model 2: Adding `species` reduces RSS significantly (from \~17,046,692 to \~4,439,967). The F-statistic (500.39) and p-value (\<2.2e-16) indicate a highly significant improvement.
...
-   Convince yourself that the three models are nested in one another, so we can use all three methods: the adjusted $R^2$, the `AIC` and the `ANOVA` method.
...
$$
Y_{mass}=b_0 + b_0^{chstrap}+ b_1^{gentoo} + (b_1 + b_1^{chstrap}+ b_1^{gentoo})\cdot X \\ 
$$
...
Y_{mass}^{Adelie}&=b_0 + b_1\cdot X \\
Y_{mass}^{chstrap}&= (b_0 + b_0^{chstrap}) + (b_1 + b_1^{chstrap})\cdot X \\
Y_{mass}^{gentoo}&=(b_0 + b_0^{gentoo}) + (b_1 + b_1^{gentoo})\cdot X
\end{aligned}
$$
