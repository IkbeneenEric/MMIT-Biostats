...
2.  Create a new data frame that includes flights headed to SFO in February, and save this data frame as `sfo_feb_flights`. How many flights meet these criteria? Try to use the function `nrow` inside of [inline code](https://rmarkdown.rstudio.com/lesson-4.html) in your answer, and knit your file to see that your text shows the  answer correctly. 
...
nycflights %>%
  ...
  ggplot(aes(origin, on_time_dep_rate)) + geom_col()
```



5.  Create a list of `origin` airports and their rate of on time departure, and then visualize the distribution of on on time departure rate across the three airports using a segmented bar plot. If you could select an airport based on on time departure percentage, which NYC airport would you choose to fly out of?
**Hint:** For the segmented bar plot, will need to map the `aes`thetic arguments as follows: `x = origin, fill = dep_type`.
