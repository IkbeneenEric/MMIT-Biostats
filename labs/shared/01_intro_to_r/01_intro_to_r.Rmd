...
knitr::include_graphics("images/01-R-and-Rstudio.png")
```
...
-   Use a link provided to connect to posit cloud.
...
knitr::include_graphics("images/01-r-interface-2023.jpg")
```
...
knitr::include_graphics("images/01-global-options.jpg")
```
...
knitr::include_graphics("images/01-packages-tidy.jpg")
```
...
We will be using R Markdown to create reproducible lab reports. The file that we use to create the report has the extension `*.qmd`, and consists of three different parts: 

```{r img-Rmarkdown-parts, results='asis', echo=FALSE}
knitr::include_graphics("images/01-Rmarkdown-file.jpg")
```
...
knitr::include_graphics("images/01-run-single-line.jpg")
```
...
The last type of items I will cover today is data frame. Data frame is the technical jargon for data tables in R. We will cover data frames more in depth in the next lesson. Today we will just have a brief introduction. 

Say I have 3 apples, 4 banana and 2 oranges,
and I want to organize this info into a table, and called this table fruits. What do I do?
...
Sometimes it's nice to annotate your code in-line as well. The way to do it is using the `#` symbol in the code chunk. Any text on the same line after a `#` sign will be ignore in a code chunk by R.
For example:
...
You can simply add a `#` sign to the start of the lines that you want to skip, and R will ignore them. The shortcut is `Ctrl+Shift+C` on a PC and `Cmd+Shift+C` on a Mac.

2.  Insert a new code chunk and create a matrix as shown below. Save it as an object called `my_mat2`. Now do the following: (a) Take the square root for each member of `my_mat2`, then take `log2()`, and (b) lastly find the maximum value (use the pipe syntax. The command for maximum is `max()`)
...
3.  Use the following info to make a data frame and save it as an item called "grade". Now pull out the column with the scores. Use the `$` syntax.
...
The Arbuthnot data set can be loaded into your environment and then explored, manipulated and visualized using the `read_csv` function. The data is stored in an object we call `arbuthnot`. We store the data in our new object by using  an `r emo::ji("left_arrow")`.  
...
knitr::include_graphics("images/01-arbuth-env.jpg")
```



<img style="float: right; margin: 0px 0px 20px 20px" src="images/01-Arbuthnot_John.jpg" alt="Portrait of John Arbuthnot by G Kneller" width="20%" height="20%">
...
4. Load the `tidyverse` library into your environment, and then load the `arbuthnot` dataset. Now use the `glimpse` function to display its content, and the `dim` function to display its dimensions. Write down the variables associated with the dataset and the number of observations in your dataset.  
...
5.  What command would you use to extract just the counts of girls baptized? Try it out in the code chunk below:
...
knitr::include_graphics("images/01-ggplot-map.jpg")
```
...
6.  Create the plot and answer the following: is there an apparent trend in the number of girls baptized over the years? How would you describe it? 
...
7. Now, generate a plot of the proportion of boys born over time. What do you see?
