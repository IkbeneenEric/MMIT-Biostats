...
```{r test-four-models-full-ds, echo=FALSE, eval=FALSE}
# For testing purposes only

library(here)
natality_orig <- 
  read_rds(here("../natality2018-full.rds")) %>% 
  select(
    DBWT,
    CIG_REC,   #
    risks,     #
...
M.FULL_NO_INT <- lm(formula = DBWT ~ ., data = natality)

anova(M.FULL, M.FULL_NO_INT)
...
MAGER.szd <- 
  standardize(
    data.frame(MAGER = c(20, 30, 40)), 
    two.sd = TRUE, 
    reference = natality_orig
    ) |> 
  mutate(MAGER = MAGER/2) |> 
  pull(MAGER) |> 
  as.numeric()
...
mosaic::fav_stats(natality_orig$MAGER)
(c(20, 30, 40) - 28.85485) / (5.767747*2)

mosaic::fav_stats(natality$BMI)
...
predictions(M.FULL, 
  ...
  select(rowid, estimate, MRACEHISP, BMI, MAGER) |> 
  as.data.frame() |> 
  pivot_wider(
    id_cols = c(MRACEHISP, MAGER), 
    names_from = BMI, 
    names_prefix = "BMI_", 
    values_from = estimate
    ) %>% mutate(
      MAGER_unst = case_match(
        MAGER, 
        MAGER.szd[1] ~ "20", 
        MAGER.szd[2] ~ "30", 
        MAGER.szd[3] ~ "40", 
  ...
  ggplot() + 
  geom_point(
    aes(x = "mean(BMI)", y = BMI_0, color = MAGER_unst), 
    size = 5
    ) +
  geom_point(
    aes(x = "mean(BMI) + sd(BMI)", y = BMI_0.5, color = MAGER_unst), 
    size = 5
    ) +
  geom_segment(
    aes(x = "mean(BMI)", 
  ...
  theme_minimal() + 
  theme(legend.position = "bottom", text = element_text(size = 14)) +
  facet_wrap(~MRACEHISP, scale = "free")
